...
[See here for the code for this dashboard](https://github.com/andrewheiss/quarto-websites_2025-10/blob/main/materials/interactivity/dashboard.qmd) (and [click here](dashboard.qmd){target="_blank"} to open the dashboard in a new tab):
...
Live `rnorm()` ([see code here](https://github.com/andrewheiss/quarto-websites_2025-10/blob/main/materials/interactivity/dashboard.qmd)):
...
#| input:
#|   - n
hist(rnorm(n), main = paste0("rnorm(", n, ")"))
```
...
[Original Shiny k-means example](https://shiny.posit.co/r/gallery/start-simple/kmeans-example/)

Recreation with webR and OJS inputs ([see code here](https://github.com/andrewheiss/quarto-websites_2025-10/blob/main/materials/interactivity/dashboard.qmd)):
...
clusters_result <- kmeans(selectedData, clusters)
...
plot(
  selectedData,
  col = clusters_result$cluster,
  pch = 20, cex = 3,
  xlab = xcol, ylab = ycol
)
points(clusters_result$centers, pch = 4, cex = 4, lwd = 4)
```
